...
class Request(object):
    ...
    def using(self, client):
        ...
        :arg client: an instance of ``elasticsearch.Elasticsearch`` to use or
            an alias to look up in ``elasticsearch_dsl6.connections``
...
class MultiSearch(Request):
    """
    Combine multiple :class:`~elasticsearch_dsl6.Search` objects into a single
    request.
    """
    def __init__(self, **kwargs):
    ...
    def add(self, search):
        """
        Adds a new :class:`~elasticsearch_dsl6.Search` object to the request::
